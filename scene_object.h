...
struct scene_object : public box {
	...
	static gasinfo<nc> &gas() {
		static gasinfo<nc> _gas;
		return _gas;
	}
	...
	static void set_gas(const gasinfo<nc> &gas) { scene_object<nc>::gas() = gas; }
	static void set_gravity(const vec &g) { scene_object<nc>::g() = g; }

	scene_object(int nx, int ny, int nz, const vec &ll, const vec &ur, const std::string &id)
		: box(nx, ny, nz, ll, ur, id)
	{
